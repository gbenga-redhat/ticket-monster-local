...
public class BookingService extends BaseEntityService<Booking> {
	...
	/**
	 * The BookingService delegates to the SeatAllocationService to find seats in the requested section,
	 *  the required SeatAllocationService instance is initialized and supplied by the container as needed. 
	 *  The only thing that our service does is to specify the dependency in form of an injection point - 
	 *  the field annotated with @Inject.

We would like other parts of the application to be aware of the fact that a new booking has been created, 
therefore we use the CDI to fire an event. We do so by injecting an Event<Booking> instance into the service
 (indicating that its payload will be a booking). In order to individually identify this event as referring to 
 event creation, we use a CDI qualifier, which we need to add: see
 org.jboss.examples.ticketmonster.util.qualifier.Created
 
What are qualifiers?
CDI uses a type-based resolution mechanism for injection and observers. 
In order to distinguish between implementations of an interface, you can use qualifiers,
 a type of annotations, to disambiguate. Injection points and event observers can use qualifiers
